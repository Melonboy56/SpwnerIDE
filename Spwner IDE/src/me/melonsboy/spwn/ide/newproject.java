...
public class newproject {
    public static void newproject() throws IOException {
        ...
        if (output==0) {
            ...
            FileWriter fileWriter = new FileWriter(new File(projectlocationpanel_input.getText()).getPath()+"/"+Main.project_json_name);
            ...
            Main.add_recent_project_path(new File(projectlocationpanel_input.getText()).getPath());
        }
    }
    private static void apply_template(String folderpath, JSONObject template_json,JSONObject projectjson) throws IOException {
        ...
        for (int i = 0; i < template_numbers.size(); i++) {
            ...
            if (((String)indexjsonobject.get("type")).equalsIgnoreCase("folder")) {
                ...
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("compilerversion",(indexjsonobject.get("version")));
                jsonObject.put("path",(indexjsonobject.get("path")));
                projectjson.putIfAbsent("compileconfigs",new JSONObject());
                ((JSONObject)projectjson.get("compileconfigs")).put(indexjsonobject.get("name"),jsonObject);
            } else if (((String)indexjsonobject.get("type")).equalsIgnoreCase("selectcompileconfig")) {
                projectjson.put("selected_compile_config",indexjsonobject.get("name"));
            }
        }
    }
}
